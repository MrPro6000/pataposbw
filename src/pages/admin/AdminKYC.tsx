...
const AdminKYC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "approved":
        return <span className="px-2 py-1 bg-green-500/20 text-green-500 rounded-full text-xs">Approved</span>;
      case "rejected":
        return <span className="px-2 py-1 bg-red-500/20 text-red-500 rounded-full text-xs">Rejected</span>;
      default:
        return <span className="px-2 py-1 bg-yellow-500/20 text-yellow-500 rounded-full text-xs">Pending</span>;
    }
  };
    ...
    <AdminLayout>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            placeholder="Search by Omang, email, or name..."
            value={search}
