...
const QuickActionButton = ({ 
  ...
  <button className={`flex flex-col items-center justify-center gap-2 p-4 rounded-2xl transition-all ${
    variant === "dark" 
...
const MobileDashboardHome = () => {
  ...
  const chartConfig = {
    value: { label: "Sales", color: "#E8E8E8" },
  };

  // Mock sales history
  const salesHistory = [
    { type: "Card", status: "Approved", amount: -12.00, icon: CreditCard },
    { type: "Card", status: "Refunded", amount: 12.00, icon: CreditCard },
    { type: "Card", status: "Approved", amount: -5.00, icon: CreditCard },
  ];
        ...
        <div className="bg-white rounded-2xl">
          ...
          {salesHistory.length === 0 ? (
            <div className="px-5 py-8 text-center">
              <p className="text-[#141414]/60 text-sm">No sales yet</p>
            </div>
          ) : (
                ...
                <div key={index} className="flex items-center justify-between px-5 py-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      sale.status === "Approved" ? "bg-[#141414]/5" : "bg-gray-100"
                    }`}>
                    ...
                    <div>
                      ...
                      <p className={`text-sm ${
                        sale.status === "Approved" ? "text-[#141414]/60" : "text-[#141414]/60"
                      }`}>
                        {sale.status}
                      </p>
                    </div>
                  ...
                  <p className={`font-semibold ${
                    sale.amount < 0 ? "text-[#141414]" : "text-green-600"
                  }`}>
