...
const MobileHubView = ({ profile, userEmail }: MobileHubViewProps) => {
  ...
  // Build quick access items based on preferences
  const quickAccessItems = [
    { key: "sell", show: preferences.show_sell_products, content: (
      <button key="sell" onClick={() => setPosOpen(true)}
        ...
        <div><p className="font-semibold text-foreground text-sm">PataPOS</p><p className="text-xs text-muted-foreground">Sell from your phone</p></div>
      </button>
    )},
    { key: "devices", show: preferences.show_devices, content: (
      <Link key="devices" to="/dashboard/devices"
        ...
        <div><p className="font-semibold text-foreground text-sm">Card machine</p><p className="text-xs text-muted-foreground">4 devices</p></div>
      </Link>
    )},
    { key: "gateway", show: true, content: (
      <button key="gateway" onClick={() => setPaymentGatewayOpen(true)}
        ...
        <div><p className="font-semibold text-foreground text-sm">Payment gateway</p><p className="text-xs text-muted-foreground">Online payments</p></div>
      </button>
    )},
    { key: "mukuru", show: preferences.show_mukuru, content: (
      <button key="mukuru" onClick={() => setMoneyTransferOpen(true)}
        ...
        <div><p className="font-semibold text-foreground text-sm">Send Money</p><p className="text-xs text-muted-foreground">International transfers</p></div>
      </button>
    )},
    { key: "mobile_money", show: preferences.show_mobile_money, content: (
      <button key="mobile_money" onClick={() => setLoanApplicationOpen(true)}
        ...
        <div><p className="font-semibold text-foreground text-sm">Mobile Money</p><p className="text-xs text-muted-foreground">Orange, Smega, MyZaka</p></div>
      </button>
    )},
    { key: "wallet", show: true, content: (
    ...
    <div className="min-h-screen bg-gradient-to-b from-muted to-pata-cream dark:from-background dark:to-background pb-24">
      ...
      {preferences.show_reports && (
        <div className="px-5 py-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setProductReportOpen(true)}
            ...
            <button onClick={() => setStaffReportOpen(true)}
      ...
      {quickAccessItems.length > 0 && (
        <div className="px-5 py-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {quickAccessItems.map(item => item.content)}
          </div>
        </div>
      )}

      {/* Report Sheets */}
      <MobileReportSheet open={productReportOpen} onClose={() => setProductReportOpen(false)} reportType="product" />
      <MobileReportSheet open={staffReportOpen} onClose={() => setStaffReportOpen(false)} reportType="staff" />
      <MobileReportSheet open={revenueReportOpen} onClose={() => setRevenueReportOpen(false)} reportType="revenue" />
      ...
      <MobilePOSSheet open={posOpen} onClose={() => setPosOpen(false)} />
